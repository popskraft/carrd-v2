 ...
 *
 * Usage:
 *   node sync-clean-embeds.mjs --dry-run --backup-dir <dir> --out-json <file>
 *   node sync-clean-embeds.mjs --write   --backup-dir <dir> --out-json <file>
 */
...
const TARGETS = [
  { id: "embed02", label: "Cookie Banner", file: "dist/cookie-banner/cookie-banner-embed.html" },
  { id: "embed03", label: "FAQ", file: "dist/faq/faq-embed.html" },
  { id: "embed07", label: "Modal", file: "dist/modal/modal-embed.html" },
];
...
async function main() {
  ...
  const checks = (result.after || []).map((entry, i) => {
    ...
    return {
      ...
      beforeCleanFaqScope: (result.before[i] && /\[data-faq\]/.test(result.before[i].content || "")) || false,
    };
  });
