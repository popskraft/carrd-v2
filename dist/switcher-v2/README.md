...
1. Open `switcher-v2-cdn.html` from this folder.
...
1. Open `switcher-v2-embed.html` from this folder.
2. Copy the full contents.
...
2. Open its attributes panel and add: `data-switcher-v2=pricing`
3. Add the elements you want to switch.
4. Open each target element's attributes panel and add: `data-switcher-v2-target=pricing`.
...
1. Add a **Buttons** element and set attribute: `data-switcher-v2=pricing`
2. Add the elements you want to switch.
3. Add `data-switcher-v2-target=pricing` to every target element.
4. Add `data-switcher-v2-index=1` to every element shown by the first button.
5. Add `data-switcher-v2-index=2` to every element shown by the second button.
...
1. Add a **Buttons** element and set two attributes:
   - `data-switcher-v2=cases`
   - `data-switcher-v2-mode=cluster`
2. Add a target attribute to each section or container that should be switched:
   - `data-switcher-v2-cluster=cases`
...
Use a unique `data-switcher-v2` name for each independent switcher on the page.
...
- A controller is any **Buttons** element with `data-switcher-v2="..."`.
- Targets are matched either by `data-switcher-v2-target` / `data-switcher-v2-index` or by cluster mode attributes.
- The first target is shown by default unless `defaultIndex` changes it.
- Multiple controllers with the same `data-switcher-v2` name stay synchronized.
...
If nothing switches, check that the `data-switcher-v2` value and the `data-switcher-v2-target` or cluster attribute value match exactly.
...
`instances` is optional. Use it when two different switchers on the same page need different behavior. The key must match the `data-switcher-v2` value:

- `instances.price` applies only to `data-switcher-v2="price"`;
- `instances.cases` applies only to `data-switcher-v2="cases"`;
...
| `controllerSelector` | `[data-switcher-v2]` | Selector used to find switcher controllers |
...
| `targetAttribute` | `data-switcher-v2-target` | Attribute used to find v2 data targets |
| `targetIndexAttribute` | `data-switcher-v2-index` | Attribute used to map v2 targets to button indexes |
| `modeAttribute` | `data-switcher-v2-mode` | Attribute used to select `class-index` or `cluster` |
| `clusterTargetAttribute` | `data-switcher-v2-cluster` | Attribute used by cluster mode targets |
...
| `instances` | `{}` | Per-`data-switcher-v2` option overrides |
...
```javascript
window.CarrdSwitcherV2.show('switcher', 2);
window.CarrdSwitcherV2.next('switcher');
window.CarrdSwitcherV2.prev('switcher');
window.CarrdSwitcherV2.refresh();
```
