...
1. Open `accordeon-v2-cdn.html` from this folder.
...
1. Open `accordeon-v2-embed.html` from this folder.
2. Copy the full contents.
...
2. Set the button link to `#data-accordeon-v2-ppf`.
3. Add `data-accordeon-v2=ppf` to each container that should open and close together.
4. Publish the page and refresh.

Replace `ppf` with any short name you want to use for that group.

The plugin only handles links that match `#data-accordeon-v2-...` and have matching targets on the page. Other hash links keep their normal Carrd behavior.
...
- opens all containers with the same `data-accordeon-v2` value;
- closes them again on the next click;
- syncs any other buttons with the same `#data-accordeon-v2-...` link.
...
```html
<a href="#data-accordeon-v2-ppf" role="button">Open details</a>
...
| `hashPrefix` | `#data-accordeon-v2-` | v2 link prefix that activates toggles |
| `legacyHashPrefix` | `#accordeon-` | Legacy link prefix kept during migration |
| `linkSelector` | auto | Selector used to find toggle links |
| `targetAttributes` | `['data-accordeon-v2','data-accorderon-v2']` | Target attributes checked for matching containers |
...
```javascript
window.CarrdAccordeonV2.open('ppf');
window.CarrdAccordeonV2.close('ppf');
window.CarrdAccordeonV2.toggle('ppf');
window.CarrdAccordeonV2.refresh();
```
