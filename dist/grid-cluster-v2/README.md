...
1. Open `grid-cluster-v2-cdn.html` from this folder.
...
1. Open `grid-cluster-v2-embed.html` from this folder.
2. Copy the full contents.
...
Groups consecutive Carrd containers into a responsive CSS grid. Add one `data-grid-v2` name to each container — the plugin handles wrapping and responsive layout automatically.

No coding required. Use `data-grid-v2` and `data-grid-v2-columns` and the plugin does the rest.
...
3. Open each container's attributes panel and add the same group name, for example `data-grid-v2=features`.
4. Add `data-grid-v2-columns=3` to the first container when you want an explicit desktop column count.

All consecutive containers with the same `data-grid-v2` value form one grid. All gap and responsive controls live on the **first container** in the cluster.

---

## How It Works in Carrd

- Consecutive containers with the same `data-grid-v2` name are wrapped into one responsive grid.
- The first container acts as the cluster controller for columns, gaps, and responsive overrides.
- New setups should use `data-grid-v2-gap` and `data-grid-v2-gap-mobile` for spacing.
...
If it stays single-column, check that the containers are consecutive and the `data-grid-v2` value matches exactly.
...
| `gridAttribute` | `data-grid-v2` | Attribute used to detect v2 clusters |
...
|-------|-------------|
| `data-grid-v2-sm=1` ... `data-grid-v2-sm=6` | Sets mobile columns at `<=736px` |
| `data-grid-v2-md=1` ... `data-grid-v2-md=6` | Sets tablet / small desktop columns at `737px-1024px` |
| `data-grid-v2-lg=1` ... `data-grid-v2-lg=6` | Sets large desktop columns at `>=1280px` |
| `data-grid-v2-justify=true` | Stretches the cluster edge-to-edge |
| `data-grid-v2-width=50%` | Sets custom desktop width for one item |

Example:

```txt
data-grid-v2-sm=2 data-grid-v2-md=3 data-grid-v2-columns=4 data-grid-v2-lg=5
```
...
| Attribute | What it does |
|-----------|-------------|
| `data-grid-v2-gap=VALUE` | Row and column gap for all breakpoints. Plain number = rem, or any CSS value |
| `data-grid-v2-gap-mobile=VALUE` | Row and column gap on mobile only (≤736px). Falls back to `data-grid-v2-gap` if not set |

Examples:
- `data-grid-v2-gap=1.5` → `1.5rem` row and column gap on all breakpoints
- `data-grid-v2-gap=2 data-grid-v2-gap-mobile=0.75` → `2rem` desktop, `0.75rem` mobile
- `data-grid-v2-gap=0` → no gap
...
`data-grid-v2-gap` on a specific cluster overrides these tokens for that cluster only.
