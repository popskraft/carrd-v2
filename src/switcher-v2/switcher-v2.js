(function() {
  ...
  function buildInstance(controller) {
    ...
    if (!switcherName) {
      warn(config, 'empty data-switcher-v2 value; controller skipped', controller);
      return null;
    }

    if (!safeNamePattern.test(switcherName)) {
      warn(config, `invalid data-switcher-v2 value "${switcherName}"; controller skipped`, controller);
      return null;
    }
