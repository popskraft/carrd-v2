...
- Q010 → канонический атрибут `data-stacker` подтверждён как единственная активная форма; legacy alias `data-stacked` в runtime сохранён для внешних/устаревших разметок, но больше не используется в active markup. `main-template` Builder мигрирован live (`container22`/`container23`/`container24`, группа `stack`): `data-stacked=stack` → `data-stacker=stack`, изменения проверены readback'ом и висят в Builder-драфте (Publish помечен как dirty), ждут operator publish.
