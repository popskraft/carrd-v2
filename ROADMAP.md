...
## Суть
`carrd-v2` закрепил split theme/runtime contract: новые installs идут через `theme-runtime` + полный `theme-design-tokens-embed.html`, а `theme-core` / `theme-ui.css` оставлены только как controlled compatibility tails для существующих mutable `@main` consumers. Следующий operator-gated шаг — обновлять live Carrd installs на version-pinned runtime path и затем продолжать миграцию `main-template` -> `faktura` -> export/install в `faktura-app`.
